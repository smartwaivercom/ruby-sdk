...
begin
  result = client.get_template_message(template_id)
  message = result[:api_webhook_template_message_get]

  if (message.nil?)
    puts "No messages in template queue"
    Kernel.exit(1)
  end


  puts "Message in Template Queue"
  ...
  delete_result = client.delete_template_message(template_id, message[:messageId])
